...
function M.start(opts)
  ...
  vim.ui.input({ prompt = "0x0 edit> " }, function(instruction)
    ...
    -- Save all buffers first to capture current state
    vim.cmd("silent! wall")
    ...
    snapshot.create(function(err, stash_ref)
      ...
      -- Step 2: Build prompt and send to server
      ...
      _state.abort_fn = api.stream_message(cfg.server_url, body, {
        ...
        on_done = function()
          ...
          -- Reload buffers to see agent's changes
          vim.cmd("checktime")
...
function M.abort()
  ...
  if _state.stash_ref then
    vim.notify("0x0: Restoring files from snapshot...", vim.log.levels.INFO)
    snapshot.get_changed_files(_state.stash_ref, function(err, files)
      ...
      for _, f in ipairs(files) do
        snapshot.restore_file(_state.stash_ref, f, function(restore_err)
