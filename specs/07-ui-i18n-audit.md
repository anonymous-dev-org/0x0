...
## Decision: How We Should Add i18n To `@anonymous-dev/ui`
...
- Export them from `@anonymous-dev/ui` via `packages/ui/package.json` exports (e.g. `"./i18n/*": "./src/i18n/*.ts"`).
...
- `packages/app/`:
  ...
  - Add a tiny bridge provider in `packages/app/src/app.tsx` to feed `useLanguage()` into `@anonymous-dev/ui`'s `I18nProvider`.
...
- `packages/enterprise/`:
  ...
  - Merge `@anonymous-dev/ui` dictionaries and (optionally) enterprise-local dictionaries.
...
1. Completed (2026-01-20): Add `@anonymous-dev/ui` i18n context (`packages/ui/src/context/i18n.tsx`) + export it.
