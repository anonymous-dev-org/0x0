...
## Decision: How We Should Add i18n To `@0x0-ai/ui`
...
- Export them from `@0x0-ai/ui` via `packages/ui/package.json` exports (e.g. `"./i18n/*": "./src/i18n/*.ts"`).
...
- `packages/app/`:
  ...
  - Add a tiny bridge provider in `packages/app/src/app.tsx` to feed `useLanguage()` into `@0x0-ai/ui`'s `I18nProvider`.
...
- `packages/enterprise/`:
  ...
  - Merge `@0x0-ai/ui` dictionaries and (optionally) enterprise-local dictionaries.
...
1. Completed (2026-01-20): Add `@0x0-ai/ui` i18n context (`packages/ui/src/context/i18n.tsx`) + export it.
