...
- `init.lua`
  - `:ZeroChatAcceptAll` / `:ZeroChatDiscardAll` semantics updated; commands
    stay.
  - `:ZeroChatReview` either renames to `:ZeroChatChanges` (jump to changes
    panel) or is removed — diffview tab is gone.
...
- `:ZeroChatReview`, `:ZeroReviewAcceptAll`, `:ZeroReviewDiscardAll`,
