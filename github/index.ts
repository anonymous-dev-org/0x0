...
const { client, server } = createZeroxzero()
...
try {
  ...
  await assertZeroxzeroConnected()
  ...
  console.log("zeroxzero session", session.id)
  if (shareId) {
...
function createZeroxzero() {
  const host = "127.0.0.1"
  const port = 4096
  const url = `http://${host}:${port}`
  const proc = spawn(`zeroxzero`, [`serve`, `--hostname=${host}`, `--port=${port}`])
  const client = createZeroxzeroClient({ baseUrl: url })
...
function assertPayloadKeyword() {
  ...
  if (!body.match(/(?:^|\s)(?:\/zeroxzero|\/oc)(?=$|\s)/)) {
    throw new Error("Comments must mention `/zeroxzero` or `/oc`")
  }
}
...
async function assertZeroxzeroConnected() {
  let retry = 0
  let connected = false
  do {
    try {
  ...
  if (!connected) {
    throw new Error("Failed to connect to zeroxzero server")
  }
}
...
function useShareUrl() {
  return isMock() ? "https://dev.zeroxzero.ai" : "https://zeroxzero.ai"
}
...
async function getAccessToken() {
  ...
  let response
  if (isMock()) {
    response = await fetch("https://api.zeroxzero.ai/exchange_github_app_token_with_pat", {
      method: "POST",
      headers: {
    ...
    const oidcToken = await core.getIDToken("zeroxzero-github-action")
    response = await fetch("https://api.zeroxzero.ai/exchange_github_app_token", {
      method: "POST",
      headers: {
...
async function getUserPrompt() {
  ...
  let prompt = (() => {
    const body = payload.comment.body.trim()
    if (body === "/zeroxzero" || body === "/oc") {
      if (reviewContext) {
        return `Review this code change and suggest improvements for the commented lines:\n\nFile: ${reviewContext.file}\nLines: ${reviewContext.line}\n\n${reviewContext.diffHunk}`
      }
      return "Summarize this thread"
    }
    if (body.includes("/zeroxzero") || body.includes("/oc")) {
      if (reviewContext) {
...
async function chat(text: string, files: PromptFiles = []) {
  console.log("Sending message to zeroxzero...")
...
async function configureGit(appToken: string) {
  ...
  await $`git config --global user.name "zeroxzero-agent[bot]"`
  await $`git config --global user.email "zeroxzero-agent[bot]@users.noreply.github.com"`
}
...
function generateBranchName(type: "issue" | "pr") {
  const timestamp = new Date()
    ...
    .split("T")
    .join("")
  return `zeroxzero/${type}${useIssueId()}-${timestamp}`
}
...
function footer(opts?: { image?: boolean }) {
  ...
  const image = (() => {
    ...
    return `<a href="${useShareUrl()}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/zeroxzero-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
  })()
  const shareUrl = shareId ? `[zeroxzero session](${useShareUrl()}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
