          ...
          <box
            ...
            <box flexDirection="row" flexShrink={0} paddingTop={1} gap={1}>
              <text fg={line()}>
                {store.mode === "shell"
                  ? "Shell"
                  : local.agent.currentMode()
                    ? `${local.agent.label(local.agent.current().name)} · ${local.agent.currentMode()}`
                    : local.agent.label(local.agent.current().name)}{" "}
              </text>
