...
import { createZeroxzero, type ToolPart } from "@0x0-ai/sdk"
...
console.log("🚀 Starting zeroxzero server...")
const zeroxzero = await createZeroxzero({
  port: 0,
})
console.log("✅ Zeroxzero server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await zeroxzero.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new zeroxzero session...")
    const { client, server } = zeroxzero
    ...
    console.log("✅ Created zeroxzero session:", createResult.data.id)
  ...
  console.log("📝 Sending to zeroxzero:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Zeroxzero response:", JSON.stringify(result, null, 2))
