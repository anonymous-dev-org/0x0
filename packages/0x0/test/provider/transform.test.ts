...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const zeroxzero = {
      ...openaiModel,
      providerID: "zeroxzero",
      api: {
        id: "zeroxzero-test",
        url: "https://api.zeroxzero.ai",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, zeroxzero, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.zeroxzero?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.zeroxzero?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    expect(result[0].providerOptions?.zeroxzero?.itemId).toBe("msg_0x0")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.zeroxzero?.itemId).toBe("msg_0x0_part")
