...
const managedConfigDir = process.env.ZEROXZERO_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "zeroxzero.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Bun.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "zeroxzero.json") {
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Bun.file(path.join(tmp.path, "zeroxzero.json")).text()
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Bun.write(path.join(dir, "zeroxzero.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .zeroxzero directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const zeroxzero = path.join(dir, ".zeroxzero")
      await fs.mkdir(zeroxzero, { recursive: true })
      const agentDir = path.join(zeroxzero, "agent")
...
test("loads agents from .zeroxzero/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const zeroxzero = path.join(dir, ".zeroxzero")
      await fs.mkdir(zeroxzero, { recursive: true })

      const agentsDir = path.join(zeroxzero, "agents")
...
test("loads commands from .zeroxzero/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const zeroxzero = path.join(dir, ".zeroxzero")
      await fs.mkdir(zeroxzero, { recursive: true })

      const commandDir = path.join(zeroxzero, "command")
...
test("loads commands from .zeroxzero/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const zeroxzero = path.join(dir, ".zeroxzero")
      await fs.mkdir(zeroxzero, { recursive: true })

      const commandsDir = path.join(zeroxzero, "commands")
...
test("does not try to install dependencies in read-only ZEROXZERO_CONFIG_DIR", async () => {
  ...
  const prev = process.env.ZEROXZERO_CONFIG_DIR
  process.env.ZEROXZERO_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.ZEROXZERO_CONFIG_DIR
    else process.env.ZEROXZERO_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable ZEROXZERO_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        path.join(dir, "zeroxzero.json"),
        JSON.stringify({ $schema: "https://zeroxzero.ai/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "zeroxzero.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .zeroxzero config
      const projectDir = path.join(dir, "project")
      const zeroxzero = path.join(projectDir, ".zeroxzero")
      await fs.mkdir(zeroxzero, { recursive: true })
      ...
      // Local .zeroxzero config with different plugins
      await Bun.write(
        path.join(zeroxzero, "zeroxzero.json"),
        JSON.stringify({
          $schema: "https://zeroxzero.ai/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zeroxzero config with some overlapping plugins
      await Bun.write(
        path.join(zeroxzero, "zeroxzero.json"),
        JSON.stringify({
          $schema: "https://zeroxzero.ai/config.json",
...
// Note: preload.ts sets ZEROXZERO_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .zeroxzero config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .zeroxzero directory config enables it
      const zeroxzero = path.join(dir, ".zeroxzero")
      await fs.mkdir(zeroxzero, { recursive: true })
      await Bun.write(
        path.join(zeroxzero, "zeroxzero.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/zeroxzero")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/zeroxzero")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-zeroxzero@2.4.3")).toBe("oh-my-zeroxzero")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@zeroxzero/plugin@2.0.0")).toBe("@zeroxzero/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-zeroxzero@2.4.3", "file:///project/.zeroxzero/plugin/oh-my-zeroxzero.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.zeroxzero/plugin/oh-my-zeroxzero.js")
  })
  ...
  test("local plugin directory overrides global zeroxzero.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const zeroxzero = path.join(projectDir, ".zeroxzero")
        const pluginDir = path.join(zeroxzero, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("ZEROXZERO_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"]
    process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .zeroxzero/ directories when flag is set", async () => {
    const originalEnv = process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"]
    process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .zeroxzero directory with a command
          const zeroxzero = path.join(dir, ".zeroxzero", "command")
          await fs.mkdir(zeroxzero, { recursive: true })
          await Bun.write(path.join(zeroxzero, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .zeroxzero should NOT be in directories list
          const hasProjectZeroxzero = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectZeroxzero).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZEROXZERO_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["ZEROXZERO_CONFIG_DIR"]
      process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["ZEROXZERO_CONFIG_DIR"]
      } else {
        process.env["ZEROXZERO_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("ZEROXZERO_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["ZEROXZERO_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["ZEROXZERO_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["ZEROXZERO_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from ZEROXZERO_CONFIG_DIR, not project
