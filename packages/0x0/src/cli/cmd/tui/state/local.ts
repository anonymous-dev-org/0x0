...
import type { Agent as AgentInfo } from "@/server/types"
  ...
  const mcp = {
    ...
    async toggle(name: string) {
      ...
      if (status?.status === "connected") {
        ...
        await sdk.client.mcp[":name"].disconnect.$post({ param: { name } } as any)
      } else {
        // Enable/Retry: connect the MCP (handles disabled, failed, and other states)
        await sdk.client.mcp[":name"].connect.$post({ param: { name } } as any)
      }
    },
  }
...
export const local: LocalState = new Proxy({} as LocalState, {
