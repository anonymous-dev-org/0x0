...
NOTE: At any point in time through this workflow you should feel free to ask the user questions or clarifications. Don't make large assumptions about user intent. The goal is to present a well researched plan to the user, and tie any loose ends before implementation begins.
      ...
      if (step > 1 && lastFinished) {
        for (const msg of sessionMessages) {
          ...
          for (const part of msg.parts) {
            ...
            part.text = [
              render(config.prompt?.reminder?.queued_user ?? QUEUED_USER_REMINDER, {
                message: part.text,
              }),
            ].join("\n")
          }
            ...
            ? [
                {
                  ...
                  content: config.prompt?.reminder?.max_steps ?? MAX_STEPS,
                },
              ]
            : []),
        ],
  ...
  async function insertReminders(input: { messages: MessageV2.WithParts[]; agent: Agent.Info; session: Session.Info }) {
    const config = await Config.get()
    const reminder = config.prompt?.reminder
    ...
    if (!Flag.ZEROXZERO_EXPERIMENTAL_PLAN_MODE) {
      if (input.agent.name === "plan") {
        userMessage.parts.push({
          ...
          type: "text",
          text: reminder?.plan ?? PROMPT_PLAN,
          synthetic: true,
        })
      }
      ...
      if (wasPlan && input.agent.name === "build") {
        userMessage.parts.push({
          ...
          sessionID: userMessage.info.sessionID,
          type: "text",
          text: reminder?.build_switch ?? BUILD_SWITCH,
          synthetic: true,
        })
      }
    ...
    if (input.agent.name !== "plan" && assistantMessage?.info.agent === "plan") {
      ...
      if (exists) {
        const part = await Session.updatePart({
          ...
          sessionID: userMessage.info.sessionID,
          type: "text",
          text: render(reminder?.build_switch_with_plan ?? BUILD_SWITCH_WITH_PLAN_REMINDER, {
            plan_path: plan,
          }),
          synthetic: true,
        })
