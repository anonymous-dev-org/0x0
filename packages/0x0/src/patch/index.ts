...
export namespace Patch {
  ...
  function parsePatchHeader(
    ...
    if (line.startsWith("*** Update File:")) {
      ...
      if (nextIdx < lines.length && nextLine && nextLine.startsWith("*** Move to:")) {
  ...
  export function maybeParseApplyPatch(
    ...
    const cmd = argv[0]
    const arg = argv[1]
    if (argv.length === 2 && cmd && arg && APPLY_PATCH_COMMANDS.includes(cmd)) {
      try {
        const { hunks } = parsePatch(arg)
        return {
    ...
    const script = argv[2]
    if (argv.length === 3 && argv[0] === "bash" && argv[1] === "-lc" && script) {
  ...
  function applyReplacements(lines: string[], replacements: Array<[number, number, string[]]>): string[] {
    ...
    for (let i = replacements.length - 1; i >= 0; i--) {
      ...
      if (!entry) continue
      const startIdx = entry[0]
      const oldLen = entry[1]
      const newSegment = entry[2]
