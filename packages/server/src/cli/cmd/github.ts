...
const AGENT_USERNAME = "zeroxzero-agent[bot]"
const AGENT_REACTION = "eyes"
const WORKFLOW_FILE = ".github/workflows/zeroxzero.yml"
      ...
      async fn() {
        {
          ...
          async function installGitHubApp() {
            ...
            // Open browser
            const url = "https://github.com/apps/zeroxzero-agent"
            const command =
...
jobs:
  zeroxzero:
    if: |
      ...
      contains(github.event.comment.body, ' /zeroxzero') ||
      startsWith(github.event.comment.body, '/zeroxzero')
...
export const GithubRunCommand = cmd({
  ...
  async handler(args) {
    await bootstrap(process.cwd(), async () => {
      ...
      const shareBaseUrl = isMock ? "https://dev.zeroxzero.ai" : "https://zeroxzero.ai"
      ...
      function normalizeOidcBaseUrl(): string {
        ...
        if (!value) return "https://api.zeroxzero.ai"
      ...
      function subscribeSessionEvents() {
        const TOOL: Record<string, [string, string]> = {
          ...
          apply_patch: ["Edit", UI.Style.TEXT_SUCCESS_BOLD],
          edit: ["Edit", UI.Style.TEXT_SUCCESS_BOLD],
          search: ["Search", UI.Style.TEXT_INFO_BOLD],
          search_remote: ["Search", UI.Style.TEXT_DIM_BOLD],
          glob: ["Glob", UI.Style.TEXT_INFO_BOLD],
          grep: ["Grep", UI.Style.TEXT_INFO_BOLD],
          list: ["List", UI.Style.TEXT_INFO_BOLD],
          lsp: ["LSP", UI.Style.TEXT_INFO_BOLD],
          question: ["Question", UI.Style.TEXT_HIGHLIGHT_BOLD],
      ...
      async function chat(message: string, files: PromptFiles = []) {
        ...
        const result = await SessionPrompt.prompt({
          ...
          // agent is omitted - server will use default_agent from config or fall back to "planner"
          parts: [
            {
      ...
      async function getOidcToken() {
        try {
          return await core.getIDToken("zeroxzero-github-action")
        } catch (error) {
      ...
      function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
        ...
        if (type === "schedule" || type === "dispatch") {
          ...
          return `zeroxzero/${type}-${hex}-${timestamp}`
        }
        return `zeroxzero/${type}${issueId}-${timestamp}`
      }
...
Co-authored-by: ${actor} <${actor}@users.noreply.github.com>"`
      ...
      function footer(opts?: { image?: boolean }) {
        const image = (() => {
          ...
          return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/zeroxzero-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
        })()
        const shareUrl = shareId ? `[zeroxzero session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
    ...
    issue(number: $number) {
      ...
      function buildPromptDataForIssue(issue: GitHubIssue) {
        ...
        return [
          ...
          "- Git push and PR creation are handled AUTOMATICALLY by the zeroxzero infrastructure after your response",
