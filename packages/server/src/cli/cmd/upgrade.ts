...
export const UpgradeCommand = {
  ...
  handler: async (args: { target?: string; method?: string }) => {
    ...
    if (method === "unknown") {
      prompts.log.error(`0x0 is installed to ${process.execPath} and may be managed by a package manager`)
    ...
    if (Installation.VERSION === target) {
      prompts.log.warn(`0x0 upgrade skipped: ${target} is already installed`)
      prompts.outro("Done")
      return
    }
