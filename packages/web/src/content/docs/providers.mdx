...
2. **Configure authentication** using one of the following methods:
   ...
   For project-specific or persistent configuration, use `0x0.yaml`:
...
`0x0.yaml` file. It is also recommended to disable session sharing.
...
```yaml title="~/.config/0x0/0x0.yaml"
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "helicone": {
...
```yaml title="0x0.yaml" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "llama.cpp": {
...
```yaml title="0x0.yaml" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "lmstudio": {
...
```yaml title="0x0.yaml" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "ollama": {
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in 0x0.yaml, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `0x0.yaml` file in your project directory:

   ```yaml title="0x0.yaml" ""myprovider"" {5-15}
   {
     "$schema": "https://0x0.ai/config.json",
     "provider": {
       "myprovider": {
...
```yaml title="0x0.yaml" {9,11,17-20}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "myprovider": {
