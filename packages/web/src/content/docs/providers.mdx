---
title: Providers
description: Using any LLM provider in Terminal Agent.
---

import config from "../../../config.mjs"
export const console = config.console

Terminal Agent uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Terminal Agent config.
...
in `~/.local/share/0x0/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your Terminal Agent
config.
...
Terminal Agent Zen is a list of models provided by the Terminal Agent team that have been
tested and verified to work well with Terminal Agent. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Terminal Agent Zen.
:::

1. Run the `/connect` command in the TUI, select 0x0, and head to [0x0.ai/auth](https://0x0.ai/auth).
...
It works like any other provider in Terminal Agent and is completely optional to use.
...
To use Amazon Bedrock with Terminal Agent:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running 0x0:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY 0x0

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile 0x0

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX 0x0
   ```
   ...
   For project-specific or persistent configuration, use `0x0.json`:
...
:::info
Using your Claude Pro/Max subscription in Terminal Agent is not officially supported by [Anthropic](https://anthropic.com).
:::
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for 0x0 to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX 0x0
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your 0x0 config.
...
2. Choose your authentication method:
   ...
   3. Name: `Terminal Agent`, Scopes: `api`
...
to store token in 0x0 auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Terminal Agent uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Terminal Agent
to only use your own GitLab-hosted instance, add the following to your
`0x0.json` file. It is also recommended to disable session sharing.
...
{
  "$schema": "https://0x0.ai/config.json",
  "plugin": ["@gitlab/0x0-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with 0x0:
...
To use Google Vertex AI with Terminal Agent:
...
2. Set the required environment variables:
   ...
   Set them while running 0x0.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id 0x0
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through 0x0, you can always configure it yourself.
...
```jsonc title="~/.config/0x0/0x0.jsonc"
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [0x0-helicone-session](https://github.com/H2Shami/0x0-helicone-session) plugin to automatically log each Terminal Agent conversation as a session in Helicone.

```bash
npm install -g 0x0-helicone-session
```
...
```json title="0x0.json"
{
  "plugin": ["0x0-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Terminal Agent conversation listed as a separate session.
...
You can configure 0x0 to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="0x0.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure 0x0 to use local models through LM Studio.

```json title="0x0.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure 0x0 to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Terminal Agent. See the [Ollama integration docs](https://docs.ollama.com/integrations/0x0) for details.
:::

```json title="0x0.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Terminal Agent:
...
3. Copy the API key for use in Terminal Agent.
...
6. **Important**: Before using cloud models in Terminal Agent, you must pull the model information locally:
...
Terminal Agent Zen is a list of tested and verified models provided by the Terminal Agent team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Terminal Agent Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Terminal Agent Zen**.

   ```txt
   /connect
   ```

3. Enter your Terminal Agent API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your 0x0 config.
...
5. You can also customize them through your 0x0 config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' 0x0
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group 0x0
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Zeroxzero:
...
You can also customize models through your 0x0 config. Here's an example of specifying provider routing order.
...
:::tip
You can use any OpenAI-compatible provider with 0x0. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in 0x0.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `0x0.json` file in your project directory:

   ```json title="0x0.json" ""myprovider"" {5-15}
   {
     "$schema": "https://0x0.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="0x0.json" {9,11,17-20}
{
  "$schema": "https://0x0.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Terminal Agent to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `0x0 auth list` to see if the credentials
...
2. For custom providers, check the 0x0 config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your 0x0 config.
