...
With Terminal Agent you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Terminal Agent uses our own `0x0` theme.
...
Terminal Agent comes with several built-in themes.
...
- Want Terminal Agent to match their terminal's appearance
...
Terminal Agent supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/0x0/themes/*.json` or `$XDG_CONFIG_HOME/0x0/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.0x0/themes/*.json`
4. **Current working directory** - Defined in `./.0x0/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/0x0/themes
vim ~/.config/0x0/themes/my-theme.json
```
...
{
  "$schema": "https://0x0.ai/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
