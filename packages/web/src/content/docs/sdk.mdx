---
title: SDK
description: Type-safe JS client for 0x0 server.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The 0x0 JS/TS SDK provides a type-safe client for interacting with the server.
Use it to build integrations and control 0x0 programmatically.
...
Create an instance of 0x0:

```javascript
import { createZeroxzero } from "@0x0-ai/sdk"

const { client } = await createZeroxzero()
```
...
You can pass a configuration object to customize behavior. The instance still picks up your `0x0.json`, but you can override or add configuration inline:

```javascript
import { createZeroxzero } from "@0x0-ai/sdk"

const 0x0 = await createZeroxzero({
  hostname: "127.0.0.1",
  port: 4096,
  config: {
...
console.log(`Server running at ${0x0.server.url}`)
...
If you already have a running instance of 0x0, you can create a client instance to connect to it:
...
```typescript
import type { Session, Message, Part } from "@0x0-ai/sdk"
```
