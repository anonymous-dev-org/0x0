---
title: Server
description: Interact with 0x0 server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `0x0 serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an 0x0 client can use.
...
```bash
0x0 serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Custom domain name for mDNS service | `0x0.local` |
| `--cors`        | Additional browser origins to allow | `[]`             |

`--cors` can be passed multiple times:

```bash
0x0 serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Set `ZEROXZERO_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `0x0`, or set `ZEROXZERO_SERVER_USERNAME` to override it. This applies to both `0x0 serve` and `0x0 web`.

```bash
ZEROXZERO_SERVER_PASSWORD=your-password 0x0 serve
```
...
When you run `0x0` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the 0x0 server to interact with 0x0 programmatically.
:::

This architecture lets 0x0 support multiple clients and allows you to interact with 0x0 programmatically.

You can run `0x0 serve` to start a standalone server. If you have the
0x0 TUI running, `0x0 serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Terminal Agent [IDE](/docs/ide) plugins.
...
The 0x0 server exposes the following APIs.
