...
There are two types of agents in Terminal Agent; primary agents and subagents.
...
Terminal Agent comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Terminal Agent comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

Terminal Agent comes with two built-in primary agents and two built-in subagents.
...
Configure agents in your `0x0.yaml` config file:
...
- Global: `~/.config/0x0/agents/`
- Per-project: `.0x0/agents/`

```markdown title="~/.config/0x0/agents/review.md"
---
...
If no temperature is specified, Terminal Agent uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Terminal Agent config and the project specific config.
...
The model ID in your Terminal Agent config uses the format `provider/model-id`. For example, if you're using [Terminal Agent Zen](/docs/zen), you would use `0x0/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `0x0 models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/0x0).
:::
...
```markdown title="~/.config/0x0/agents/docs-writer.md"
---
...
```markdown title="~/.config/0x0/agents/security-auditor.md"
---
