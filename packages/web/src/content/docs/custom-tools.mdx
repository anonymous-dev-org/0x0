---
title: Custom Tools
description: Create tools the LLM can call in 0x0.
---

Custom tools are functions you create that the LLM can call during conversations. They work alongside 0x0's [built-in tools](/docs/tools) like `read`, `write`, and `bash`.
...
- Locally by placing them in the `.0x0/tools/` directory of your project.
- Or globally, by placing them in `~/.config/0x0/tools/`.
...
```ts title=".0x0/tools/database.ts" {1}
...
```ts title=".0x0/tools/project.ts" {8}
import { tool } from "@0x0-ai/plugin"
...
```ts title=".0x0/tools/python-add.ts" {10}
import { tool } from "@0x0-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".0x0/tools/add.py")
