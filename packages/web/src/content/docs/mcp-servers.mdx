...
You can add external tools to Terminal Agent using the _Model Context Protocol_, or MCP. Terminal Agent supports both local and remote servers.
...
You can define MCP servers in your [Terminal Agent Config](https://0x0.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.

```jsonc title="0x0.jsonc" {6}
{
...
Organizations can provide default MCP servers via their `.well-known/0x0` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Terminal Agent automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Terminal Agent will:
...
If the server requires authentication, Terminal Agent will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `0x0 mcp auth <server-name>`.
...
```bash
0x0 mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, Terminal Agent will store the tokens securely in `~/.local/share/0x0/mcp-auth.json`.
...
Your MCPs are available as tools in Terminal Agent, alongside built-in tools. So you can manage them through the Terminal Agent config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Terminal Agent to your Sentry account.
