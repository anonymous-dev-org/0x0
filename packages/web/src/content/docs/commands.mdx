...
Create `.0x0/commands/test.md`:

```md title=".0x0/commands/test.md"
---
...
You can add custom commands through the Terminal Agent config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Terminal Agent [config](/docs/config):

```json title="0x0.jsonc" {4-12}
{
...
- Global: `~/.config/0x0/commands/`
- Per-project: `.0x0/commands/`

```markdown title="~/.config/0x0/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".0x0/commands/component.md"
---
...
```md title=".0x0/commands/create-file.md"
---
...
```md title=".0x0/commands/analyze-coverage.md"
---
...
```md title=".0x0/commands/review-changes.md"
---
...
```md title=".0x0/commands/review-component.md"
---
...
0x0 includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
