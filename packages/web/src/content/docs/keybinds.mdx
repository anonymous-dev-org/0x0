...
Terminal Agent has a list of keybinds that you can customize through the Terminal Agent config.
...
Terminal Agent uses a `leader` key for most keybinds. This avoids conflicts in your terminal.
...
The Terminal Agent desktop app prompt input supports common Readline/Emacs-style shortcuts for editing text. These are built-in and currently not configurable via `0x0.yaml`.
