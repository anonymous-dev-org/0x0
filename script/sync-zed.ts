...
const FORK_REPO = "anonymous-dev-org/zed-extensions"
const UPSTREAM_REPO = "zed-industries/extensions"
const EXTENSION_NAME = "zeroxzero"
...
async function main() {
  ...
  // Delete any existing branches for zeroxzero updates
  ...
  const prResult =
    await $`gh pr create --repo ${UPSTREAM_REPO} --base main --head ${FORK_REPO.split("/")[0]}:${branchName} --title "Update ${EXTENSION_NAME} to v${cleanVersion}" --body "Updating zeroxzero extension to v${cleanVersion}"`
