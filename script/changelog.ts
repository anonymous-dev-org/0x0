    ...
    .prompt(
      {
        ...
        model: { providerID: "zeroxzero", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], zeroxzero: Awaited<ReturnType<typeof createZeroxzero>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(zeroxzero, c.message)))
...
export async function buildNotes(from: string, to: string) {
  ...
  try {
    const lines = await generateChangelog(commits, zeroxzero)
    notes.push(...lines)
