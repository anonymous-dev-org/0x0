...
nix run nixpkgs#0x0           # 또는 github:anomalyco/0x0 로 최신 dev 브랜치
```
...
[agents](https://0x0.ai/docs/agents) 에 대해 더 알아보세요.

### 문서

Terminal Agent 설정에 대한 자세한 내용은 [**문서**](https://0x0.ai/docs) 를 참고하세요.
...
Terminal Agent 와 관련된 프로젝트를 진행하면서 이름에 "0x0"(예: "0x0-dashboard" 또는 "0x0-mobile") 를 포함한다면, README 에 해당 프로젝트가 Terminal Agent 팀이 만든 것이 아니며 어떤 방식으로도 우리와 제휴되어 있지 않다는 점을 명시해 주세요.
...
- 100% 오픈 소스
- 특정 제공자에 묶여 있지 않습니다. [Terminal Agent Zen](https://0x0.ai/zen) 을 통해 제공하는 모델을 권장하지만, Terminal Agent 는 Claude, OpenAI, Google 또는 로컬 모델과도 사용할 수 있습니다. 모델이 발전하면서 격차는 줄고 가격은 내려가므로 provider-agnostic 인 것이 중요합니다.
- 기본으로 제공되는 LSP 지원
- TUI 에 집중. Terminal Agent 는 neovim 사용자와 [terminal.shop](https://terminal.shop) 제작자가 만들었으며, 터미널에서 가능한 것의 한계를 밀어붙입니다.
- 클라이언트/서버 아키텍처. 예를 들어 Terminal Agent 를 내 컴퓨터에서 실행하면서 모바일 앱으로 원격 조작할 수 있습니다. 즉, TUI 프런트엔드는 가능한 여러 클라이언트 중 하나일 뿐입니다.
